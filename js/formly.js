        ...
        var functions = 
            ...
            validate : function(item)
            {
                ...
                if($(item).attr('validate')=='email')
                {
                    ...
                    var validType = 'Dirección de correo';
                }
                ...
                if(!valid) 
                {
                    ...
                    {
                        $('#' + formName).find('.formlyAlerts').append('<div class="formlyInvalid formlyAlert" id="' + alertName + '">'+ validType +' no válida '  + '</div>')
                        $('#' + alertName).fadeIn();
                    }
            ...
            require : function(item)
            {
                ...
                if(!$(item).val() || $(item).val()==$(item).attr('place')) 
                {
                    if(!$('#' + alertName).is(':visible'))
                    {
                        $('#' + formName).find('.formlyAlerts').append('<div class="formlyRequired formlyAlert" id="' + alertName + '">'+"'"+label+"'"+' es un campo obligatorio</div>')
                        $('#' + alertName).fadeIn();
                    }
                    var borderColor = $('#' + alertName).css('background-color');
                    $(item).css('border-color', borderColor);
            ...
            required : function(item)
            {
                ...
                if (document.MyForm.Provincias[0].selectedIndex==0)
                { 
                        if(!$('#' + alertName).is(':visible'))
                        {
                            $('#' + formName).find('.formlyAlerts').append('<div class="formlyRequired formlyAlert" id="' + alertName + '">'+"'"+label+"'"+' es un campo obligatorio</div>')
